accicon 2.2.17 es
    ...
    proceso
        ...
        Mientras NFDA(archi_fe) y NFDA(archi_agos) hacer
        ...
        Mientras NFDA(archi_fe)hacer
            si( reg_fe.apro = "NO") entonces
                reg_sal.dni:= reg_fe.dni;
                reg_sal.ApyNom:=reg_fe.ApyNom;
                reg_sal.email:=reg_fe.email;
                reg_sal.coleSc:=reg_fe.coleSc;
                leer_agos();
                cont:= cont + 1;
            finsi
        finMientras

        Mientras NFDA(archi_agos)hacer
            si( reg_agos.apro = "NO") entonces
                reg_sal.dni:= reg_agos.dni;
                reg_sal.ApyNom:=reg_agos.ApyNom;
                reg_sal.email:=reg_agos.email;
                reg_sal.coleSc:=reg_agos.coleSc;
